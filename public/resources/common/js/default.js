...
$(function(){
	...
	$('div.tab-menu > ul > li > a, div.tablink > ul > li > a, div.tabmenu.on > ul > li > a').on('click',function(e){
		...
		if(!($(this).parent().hasClass('active'))){
			...
			$('div.tab-menu > ul > li > a, div.tablink > ul > li > a, div.tabmenu.on > ul > li > a').removeAttr('title');
	...
	if($('div.Gnb.notype').length > 0){
		if($('div.Gnb.ge').length > 0){
	...
	$('div.lnb > ul > li').each(function(){
		if($(this).find('ul').length > 0){
			...
			if($(this).hasClass('active')){
				$(this).children('a').append('<i class="fa"></i>');
			}else{
				$(this).children('a').append('<i class="fa"></i>');
			}
...
/*
	$('div.lnb > ul > li').each(function(){
		if($(this).find('ul').length > 0){
			$(this).addClass('s');
			if($(this).hasClass('active')){
				$(this).children('a').append('<i class="fa fa-minus"></i>');
			}else{
...
*/
	...
	$('div.mmode > a, div.mmode > a#mmode').on('click', function() {
		if ( mmenuFlag ) {
			...
			$('div.mm-vertical ul.gnbMenu > li > a.mm-next').on('click', function() {
				if (!$(this).parent('li').hasClass('mm-opened')) {
					var index = $('div.mm-vertical ul.gnbMenu > li > a.mm-next').index($(this));
					$('div.mm-vertical ul.gnbMenu > li').not(index).removeClass('mm-opened');
				}
			});
...
/*
	$('select#locationSite').on('change', function() {
		if ($(this).val() != '') {
			window.open($(this).val());
		}
	});

	$('select#recommendSite').on('change', function() {
		if ($(this).val() != '') {
			window.open($(this).val());
		}
	});
*/
	$('a.recommendSite11').on('click', function() {
		if ($('select#locationSite').val() != '') {
			window.open($('select#locationSite').val());
		}
	});
});

function openLoginWindow(url) {
	var newWinLogin = window.open(url, 'loginWin', 'width=1200,height=850,scrollbars=yes');
	if (newWinLogin == null) {
		alert("팝업 차단 기능이 설정되어있습니다\n\n" +
				"차단 기능을 해제(팝업허용) 한 후 다시 이용해 주십시오.\n\n" +
				"* 차단 해제 방법 \n" +
				"설정 - 인터넷 옵션 - 개인정보 - 팝업차단 설정\n" +
				"허용할 웹 사이트 주소 : *.sen.go.kr 추가");
		return false;
	}
